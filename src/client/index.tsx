...
} satisfies Record<PromptOptimizerLocaleKey, string>
...
export type PromptOptimizerButtonProps =
  PropsRuntime<'conversation.input.right'> &
  InjectFace<PromptOptimizerButtonInjected> &
...
function SparklesIcon(): JSX.Element {
  return (
    <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none">
      <path d="M12 2l1.35 4.15L17.5 7.5l-4.15 1.35L12 13l-1.35-4.15L6.5 7.5l4.15-1.35L12 2Z" fill="currentColor" />
      <path d="M18.5 13l.92 2.58L22 16.5l-2.58.92L18.5 20l-.92-2.58L15 16.5l2.58-.92L18.5 13ZM5 12l.72 2.28L8 15l-2.28.72L5 18l-.72-2.28L2 15l2.28-.72L5 12Z" fill="currentColor" />
    </svg>
  )
}
...
export function PromptOptimizerButton({
  ...
  optimizePrompt,
  t,
}: PromptOptimizerButtonProps): JSX.Element {
  ...
  return (
    <span>
      ...
      {status.kind !== 'idle' && (
        <span className="dsh-prompt-optimizer-status" role="status">{status.text}</span>
      )}
    </span>
  )
}
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.slots.inject('conversation.input.right', () => ctx.slots.register({
    ...
    order: 20,
    locale: NS,
    inject: (sessionId: SessionId): PromptOptimizerButtonInjected => ({
      optimizePrompt: async (draft: string): Promise<string> => {
        const result = await ctx.remote.commands.execute(
          sessionId,
          `/optimize-prompt --button\n${draft}`,
        )
        if (!result.ok) throw new Error(`${result.error.message} (${result.error.code})`)
        if (result.value === undefined) throw new Error(ctx.locale.bind(NS)('error.unknown-command'))
