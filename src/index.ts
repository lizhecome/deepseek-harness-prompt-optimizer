...
import type { FinishReason, UserMessage } from '@deepseek-ai/dsh-llm'
...
export const Config: z<Config> = z.object({
  ...
  delivery: z.union(['append', 'replace'] as const).default('append'),
  failureMode: z.union(['pass-through', 'fail'] as const).default('pass-through'),
...
interface ResolvedConfig {
  configuredTarget: Target | undefined
  maxTokens: number
  minChars: number
  delivery: Delivery
  failureMode: FailureMode
...
function resolveTarget(configured: Target | undefined, agent: Agent): Target {
  ...
  throw new Error(
    'prompt-optimizer: no provider/model is available; configure both fields or give the agent an explicit route',
  )
}

/** Return text for an eligible direct-user message, otherwise leave it untouched. */
function optimizableText(message: UserMessage, minChars: number): string | undefined {
  if (message.source.kind !== 'user' || message.content.length === 0) return undefined
  if (!message.content.every(block => block.type === 'text')) return undefined
...
function finishError(finish: FinishReason): Error | undefined {
  ...
  if (finish.kind === 'max-tokens') {
    const error = new Error('prompt-optimizer: optimizer output reached `maxTokens` and may be incomplete') as Error & { code?: string }
...
async function optimize(
  ...
  const text = blocks
    ...
    .join('')
    .trim()
  if (text.length === 0) throw new Error('prompt-optimizer: optimizer returned no text')
  return text
}

/** Deliver one optimized prompt while preserving the configured transcript behavior. */
function deliver(message: UserMessage, optimized: string, delivery: Delivery): UserMessage[] {
  if (delivery === 'replace') {
    ...
    for (const message of downstream.messages) {
      ...
      try {
        const optimized = await optimize(
          ctx,
          message,
          resolveTarget(resolved.configuredTarget, agent),
          resolved,
          agent,
          signal,
        )
        ...
        if (signal.aborted || resolved.failureMode === 'fail') throw error
