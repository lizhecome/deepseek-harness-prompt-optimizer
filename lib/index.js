...
function markManualDelivery(deliveries, agent, text) {
    const pending = deliveries.get(agent) ?? new Set();
    pending.add(text);
...
function consumeManualDelivery(deliveries, agent, text) {
    const pending = deliveries.get(agent);
    if (pending === undefined || !pending.delete(text))
        return false;
    if (pending.size === 0)
        deliveries.delete(agent);
    return true;
}
...
async function executeCommand(ctx, config, deliveries, invocation) {
    const { fromButton, text } = parseCommandInput(invocation.rawInput);
    if (text.length === 0)
        return { kind: 'error', text: 'Enter a prompt to optimize.' };
    try {
        const optimized = await optimize(ctx, createUserMessage({ content: [{ type: 'text', text }], source: { kind: 'user' } }), resolveTarget(config.configuredTarget, invocation.agent), config, invocation.agent, invocation.signal);
        if (fromButton)
            markManualDelivery(deliveries, invocation.agent, optimized);
...
export function apply(ctx, config) {
    ...
    const manualDeliveries = new WeakMap();
